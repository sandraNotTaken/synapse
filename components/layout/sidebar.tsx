            ...
            return (
              <div key={link.href} className="space-y-1">
                <div className="flex items-center justify-between gap-1 group">
                  ...
                  {link.href === "/dashboard/courses" && !isCollapsed && courses && courses.length > 0 && (
                    <button
                      type="button"
                      onClick={(e) => {
                        e.preventDefault();
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 z-20 hidden h-screen flex-col border-r border-white/5 bg-[#05070d]/80 py-6 backdrop-blur-xl xl:flex transition-all duration-300 ${collapsed ? "w-20 px-3" : "w-72 px-4"}`}>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">{renderNavContent(false)}</div>
      </aside>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">{renderNavContent(true)}</div>
      </aside>
    </>
  );
}